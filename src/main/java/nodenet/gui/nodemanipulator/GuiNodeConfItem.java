package nodenet.gui.nodemanipulator;
...
import nodenet.gui.inventory.SlotWrapper;
import nodenet.gui.widget.QuickSelectorRing;
import nodenet.network.NetworkHandler;
import nodenet.network.messages.PacketServerContainerCommand;
...
public class GuiNodeConfItem extends GenericGuiContainer<GenericTileEntity> {
	...
	public GuiNodeConfItem(NodeConfItemContainer container) {
		super(NodeNet.INSTANCE, NetworkHandler.network, null, container, 0, "");
		xSize = 176;
		ySize = 133;
	...
	private void setSelected(int index) {
		...
		if(index != selectedButton) {
			NetworkHandler.network.sendToServer(new PacketServerContainerCommand(Const.COMMAND.OPTIONS, new Argument("sel", index)));
		}
